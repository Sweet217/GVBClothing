<template>
    <div>
      ...
      <section class="p-6 flex flex-wrap md:flex-nowrap gap-6">
        <!-- Outfit Builder Selection on the Left -->
        ...
        <!-- Selected Items Display on the Right -->
        <div class="w-full md:w-1/2 border rounded p-4">
          <h2 class="text-2xl font-bold mb-4">Outfit Preview</h2>
          <div class="flex flex-col items-center gap-4">
            <!-- Render each type of clothing item -->
  ...
  export default {
    ...
    data() {
      return {
        ...
        clothingLibrary: [], // Array to store uploaded items
        outfit: {
          top: [], // Array to hold multiple tops
          bottom: [], // Array to hold multiple bottoms
          shoes: [], // Array to hold multiple shoes
          accessory: [], // Array to hold multiple accessories
        },
      };
    },
    methods: {
      handleFileUpload(event) {
        ...
        if (file) {
          this.newItem.image = URL.createObjectURL(file); // Temporary image preview
        }
      },
      uploadItem() {
        if (this.newItem.type && this.newItem.image) {
          this.clothingLibrary.push({ ...this.newItem });
          this.newItem = { type: "", image: null }; // Reset form
        } else {
